...
FORCE_JOIN_STRICT = os.getenv("FORCE_JOIN_STRICT", "1").lower() not in ("0", "false", "no")
...
async def send_log(context, title, details=""):
    """Send important bot events to the configured Telegram log channel."""
    ...
    try:
        ...
        log.exception("Could not send log to channel")
...
async def dashboard_health():
    return {"ok": True, "service": "qr-claim-bot", "dashboard": True}
...
async def dashboard_proof_action(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    ...
    if telegram_bot is not None:
        try:
            ...
            pass
    if telegram_bot is not None and LOG_CHANNEL_ID:
        try:
            await telegram_bot.send_message(LOG_CHANNEL_ID, f"<b>{'✅' if action=='approve' else '❌'} DASHBOARD PROOF {action.upper()}</b>\n\nProof: <code>#{proof_id}</code>\nUser: <code>{proof['user_id']}</code>\nAdmin: <code>{admin_id}</code>", parse_mode="HTML")
        except Exception:
            log.exception("Dashboard proof log failed")
    ...
    if telegram_bot is not None:
        try:
            await telegram_bot.send_message(
                LOG_CHANNEL_ID,
                f"<b>💰 DASHBOARD BALANCE ADDED</b>\n\nUser: <code>{user_id}</code>\nAmount: ₹{amount:.2f}\nAdmin: <code>{admin_id}</code>\nReason: {reason}",
                parse_mode="HTML"
            ) if LOG_CHANNEL_ID else None
        except Exception:
            log.exception("Dashboard log failed")
        try:
            await telegram_bot.send_message(
                user_id,
                f"💰 <b>BALANCE ADDED</b>\n\n"
...
async def dashboard_withdrawal_action(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    ...
    if telegram_bot is not None and LOG_CHANNEL_ID:
        try:
            await telegram_bot.send_message(LOG_CHANNEL_ID, f"<b>💸 DASHBOARD WITHDRAWAL {new_status.upper()}</b>\n\nID: <code>#{wid}</code>\nUser: <code>{w['user_id']}</code>\nAmount: ₹{w['amount']:.2f}", parse_mode="HTML")
        except Exception:
            log.exception("Dashboard withdrawal log failed")
...
async def force_join_ok(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        ...
        log.warning("Force join check failed for %s: %s", FORCE_JOIN_CHANNEL, exc)
        if not FORCE_JOIN_STRICT:
            return True
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_log(context, "👤 NEW/RETURNING USER", f"User: <code>{update.effective_user.id}</code>\nUsername: @{update.effective_user.username or '—'}")
...
async def create_withdrawal(update, context, account):
    ...
    await send_log(context, "💸 WITHDRAWAL REQUEST", f"ID: <code>#{wid}</code>\nUser: <code>{uid}</code>\nAmount: ₹{amount:.2f}\nMethod: {method}")
...
async def submit(update, context):
    ...
    await send_log(context, "🧾 PROOF SUBMITTED", f"QR: <code>#{qr_id}</code>\nUser: <code>{q.from_user.id}</code>")
...
async def change_withdrawal_status(update, context):
    ...
    await send_log(context, f"{labels[new_status]} WITHDRAWAL", f"ID: <code>#{wid}</code>\nUser: <code>{w['user_id']}</code>\nAmount: ₹{w['amount']:.2f}\nAdmin: <code>{q.from_user.id}</code>")
...
async def add_balance(update, context):
    ...
    await send_log(context, "💰 BALANCE ADDED", f"User: <code>{uid}</code>\nAmount: ₹{amount:.2f}\nAdmin: <code>{update.effective_user.id}</code>\nReason: {reason}")
...
async def text_handler(update, context):
    ...
    # Ignore ordinary/unrecognized text. Do not spam the main menu.
    return
