...
async def dashboard_post_qr(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    ...
    posted=[]
    errors=[]
    markup=InlineKeyboardMarkup([[InlineKeyboardButton("💳 CLAIM QR",callback_data="claim")]])
...
async def postqr(update, context):
    """Post the next available QR to every configured group and DM all known users."""
    if update.effective_chat.type != "private" or not is_admin(update.effective_user.id):
        return
    ...
    if not posted:
        await update.message.reply_text(
            "🔴 QR could not be posted to any configured group.\n\n"
            "Check that the bot is an admin in every group and GROUP_IDS is correct.",
            reply_markup=admin_menu()
        )
        return
