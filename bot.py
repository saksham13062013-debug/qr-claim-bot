...
def admin_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("ADMINS", callback_data="admin_admins"), InlineKeyboardButton("GIFT CODES", callback_data="admin_gifts")],
...
async def rewards(update, context):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("REWARD HISTORY", callback_data="order_history")],
...
async def order_history(update, context):
    """Show every QR claimed by the user and its latest status."""
    q = update.callback_query
    await q.answer()
    ensure_user(q.from_user)

    page = max(0, int(context.user_data.get("order_page", 0)))
    per_page = 8
    offset = page * per_page

    with closing(db()) as con:
        rows = con.execute("""
            SELECT q.id, q.app_name, q.reward, q.status, q.claimed_at, q.created_at,
                   COALESCE(p.status, '') AS proof_status
            FROM qrs q
            LEFT JOIN proofs p ON p.id = (
                SELECT p2.id FROM proofs p2
                WHERE p2.qr_id=q.id AND p2.user_id=?
        ...
        total = con.execute(
            "SELECT COUNT(*) x FROM qrs WHERE claimed_by=?", (q.from_user.id,)
        ).fetchone()["x"]
    ...
    if not rows:
        lines.append("No QR orders yet.")
    else:
        for r in rows:
            status = proof_map.get(r["proof_status"], status_map.get(r["status"], str(r["status"]).upper()))
            when = r["claimed_at"] or r["created_at"] or ""
            lines.extend([
                f"<b>Order #{r['id']}</b>",
                f"App: {escape(r['app_name'])}",
                f"Reward: ${r['reward']:.2f}",
                f"Status: <b>{status}</b>",
                f"Date: {escape(str(when))}",
                "",
            ])

    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("PREVIOUS", callback_data="order_prev"))
    if offset + per_page < total:
        nav.append(InlineKeyboardButton("NEXT", callback_data="order_next"))

    kb = []
    if nav:
        kb.append(nav)
    kb.append([InlineKeyboardButton("BACK TO MENU", callback_data="home")])
