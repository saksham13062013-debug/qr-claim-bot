...
async def dashboard_withdrawal_action(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    ...
    try:
        ...
        if telegram_bot:
            ...
            await telegram_bot.send_message(w["user_id"],msg,parse_mode="HTML",reply_markup=user_menu())
...
def user_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("💸 WITHDRAW", callback_data="withdraw"), InlineKeyboardButton("📜 TRANSACTIONS", callback_data="transactions")],
        [InlineKeyboardButton("👥 REFERRAL", callback_data="referral"), InlineKeyboardButton("🏆 LEADERBOARD", callback_data="leaderboard")],
        [InlineKeyboardButton("🎁 DAILY BONUS", callback_data="daily_bonus"), InlineKeyboardButton("🎟️ GIFT CODE", callback_data="gift")],
        [InlineKeyboardButton("👤 PROFILE", callback_data="profile"), InlineKeyboardButton("ℹ️ HELP", callback_data="help")]
    ])
...
async def claim(update, context):
    ...
    try:
        ...
        await q.edit_message_text(
            group_text, parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[claim_name_button(uid, name)]])
        )
        await send_log(context.bot, f"🎯 <b>QR CLAIMED</b>\nQR: #{qr['id']}\nUser: {escape(name)}\nID: <code>{uid}</code>")
...
async def postqr(update, context):
    ...
    with closing(db()) as con:
        ...
        users=[r["id"] for r in con.execute("SELECT id FROM users WHERE banned=0").fetchall()]
        con.commit()
    sent=failed=0
    for target in users:
        try:
            await context.bot.send_message(target, f"🎯 <b>QR CLAIM AVAILABLE</b>\n\n📱 App: {escape(qr['app_name'])}\n💰 Reward: ${qr['reward']:.2f}\n\nFirst eligible user gets this QR in DM.", parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💳 CLAIM QR", callback_data="claim")]]))
            sent+=1
        except Exception:
            failed+=1
    await send_log(context.bot, f"📢 <b>QR POSTED</b>\nAdmin: <code>{update.effective_user.id}</code>\nQR: #{qr['id']}\nDM sent: {sent}, failed: {failed}")

    await update.message.reply_text(
        f"✅ Posted to group.\n\n📨 DM claim buttons sent: {sent}\n❌ Failed: {failed}\n\n"
...
async def text_handler(update, context):
    ...
    if stage == "amount":
        ...
        if inr < MIN_WITHDRAWAL or inr > r["balance"]*USD_TO_INR:
            context.user_data.clear()
            await update.message.reply_text(f"🔴 Withdrawal not started.\n\nMinimum: ₹{MIN_WITHDRAWAL:.0f}\nMaximum available: ₹{r['balance']*USD_TO_INR:.2f}",reply_markup=user_menu()); return
    ...
    if context.user_data.get("gift_stage"):
        ...
        with closing(db()) as con:
            ...
            if not g or g["uses"]>=g["max_uses"] or con.execute("SELECT 1 FROM gift_redemptions WHERE code=? AND user_id=?",(code,update.effective_user.id)).fetchone():
                context.user_data.clear(); await update.message.reply_text("🔴 Invalid, exhausted or already-used gift code.",reply_markup=user_menu()); return
        ...
        context.user_data.clear(); await update.message.reply_text(f"🟢 Gift code redeemed! ${g['amount']:.2f} added.",reply_markup=user_menu()); return
        ...
        for target in users:
            try:
                await context.bot.send_message(
                    ...
                    parse_mode="HTML",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💳 CLAIM QR", callback_data="claim")]])
                )
                sent += 1
            except Exception:
                failed += 1
